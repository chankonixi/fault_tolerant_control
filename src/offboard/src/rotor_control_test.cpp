...
int main(int argc, char **argv)
{
  ...
  while (ros::ok())
  {
    ...
    for (int i=0; i<4;i++) {
      ...
      //缺少
      rotor_control_msg_.rotor_thrust[i] = current_state.rotor_thrust[i];
    } 
    //缺少
    rotor_control_msg_.control_mode = current_state.control_mode;
    rotor_control_msg_.thrust = current_state.thrust;
    //缺少
